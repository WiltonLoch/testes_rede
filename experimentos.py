...
def rajadas_sl(rede, carga, execucoes = 1, intervalo = 0, aquecimento = 0, carga_paralelo = ''):
    resultados = []
    for i in range(1, execucoes + 1):
        ...
        intermediario = Testes.emitir_sl(rede, carga, 0, 10, 'dados_brutos/' + carga_paralelo + '/' + str(i) + '_' + carga)
...
def experimento():
    ...
    Path('dados_brutos/baseline').mkdir(parents = True, exist_ok = True)
    for i in cargas_sl:
        print("carga: " + i)
        saida = open('resultados/baseline/%s' % i, 'w+')
        resultados = rajadas_sl(rede, i, execucoes, intervalo, aquecimento, 'baseline')
        for j in resultados:
            saida.write('%s\n' % str(j).split(':')[2])
    ...
    for i in cargas_trafego:
        ...
        Path('dados_brutos/%s' % i).mkdir(parents = True, exist_ok = True)
        for j in cargas_sl:
            print("Carga de teste: " + j)
            saida = open('resultados/%s/%s' % (i, j), 'w+')

            Testes.iperfMultiNos(rede, i, 'bisection', tempo = 1000)
            resultados = rajadas_sl(rede, j, execucoes, intervalo, aquecimento, i)
