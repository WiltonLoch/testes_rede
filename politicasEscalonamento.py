                ...
                for j in range(i, len(rede.hosts)):
                    if i != j:
                        ...
                        if choice_additional_load < smallest_load:
                            ...
                            best_choice = (i, j)
            if not (best_choice[0] + best_choice[1] in link_loads):
                link_loads[best_choice[0] + best_choice[1]] = (int(carga[:-1]) * multiplier)/link_capacity
            else:
                link_loads[best_choice[0] + best_choice[1]] += (int(carga[:-1]) * multiplier)/link_capacity
            escalonamento.append(best_choice)
        return escalonamento
