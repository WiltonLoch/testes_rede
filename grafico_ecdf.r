...
simbolos = 0:5
rev(simbolos)
ecdfplot(~ c1K + c10K + c100K + c1M + c10M + c20M, dados1G, xlab = "FCT(s)", ylab = "CDF(%)", par.settings = list(fontsize = list(text = 11), superpose.line = list(col = cores), lwd=1.5, lty=1.5), main = list(label = "CDF of FCT - 100% additional link occupation", cex = 1.5), auto.key = list(text = c('1KB', '10KB', '100KB', '1MB', '10MB', '20MB'), title = "Flow size", space = 'right'), scales = list(x = list(log = 2)), xscale.components = xscale.components.log)
#ecdfplot(~ c1K + c10K + c100K + c1M + c10M + c20M, dados250M, xlab = "FCT(s)", ylab = "CDF(%)", par.settings = list(fontsize = list(text = 11), superpose.line = list(col = cores), lwd=1.5, lty=1.5), main = list(label = "CDF of FCT - 25% additional link occupation", cex = 1.5), auto.key = list(text = c('1KB', '10KB', '100KB', '1MB', '10MB', '20MB'), title = "Flow size", space = 'right'))
